...
class MultiGraphDataset():
    ...
    def __init__(self, graph_data_path,
        ...
        h = self.node2id['PersonY be kill']
        r = self.rel2id['Precedence']
        t = self.node2id['PersonY be in a fight']
        print(h)
        print(r)
        print(t)
        print(positive_edges['trn'][:10])
        print([h, t, r] in positive_edges['trn'])
        print([h, t, r] in positive_edges['tst'])
        print([h, t, r] in positive_edges['dev'])
        print('####')
        print([h, t, r] in positive_edges['trn'])
        print([h, t, r] in positive_edges['tst'])
        print([h, t, r] in positive_edges['dev'])
        print('####')
        print([h, t, r] in negative_edges['trn'])
        print([h, t, r] in negative_edges['tst'])
        print([h, t, r] in negative_edges['dev'])
        print('####')
        print([h, t, r] in negative_edges['trn'])
        print([h, t, r] in negative_edges['tst'])
        print([h, t, r] in negative_edges['dev'])
                    ...
                    if isinstance(feat["relation"], dict):
                        ...
                        if edge not in val_edges_dict and edge not in test_edges_dict:
                            ...
                            if aser_rel_format == "highest_aser_rel":
                                ...
                                if len(sorted_rel_weight) >= 2 and sorted_rel_weight[0][0] == self.rel2id[
                                    "Co_Occurrence"]:
        ...
        mm = self.adj_list[self.node2id['PersonY be kill']]
        for n1, p1 in mm:
            if n1 == self.node2id['PersonY be in a fight']:
                print(('PersonY be kill', self.id2rel[p1], 'PersonY be in a fight'))

        mm = self.adj_list[self.node2id['PersonY be in a fight']]
        for n1, p1 in mm:
            if n1 == self.node2id['PersonY be kill']:
                print(('PersonY be in a fight', self.id2rel[p1], 'PersonY be kill'))
    ...
    def tokenize_nodes(self, node_token_path):
        if os.path.exists(node_token_path):
            ...
            self.id2nodestoken = {self.node2id[line]: torch.tensor(self.tokenizer.encode(line,
                                                                                         add_special_tokens=True)) for
                                  line in tqdm(self.node2id)}
