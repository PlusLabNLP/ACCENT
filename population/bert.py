...
class BERTForPopulation(nn.Module):
    """
    h, r, t are fet separately to the encoder, their embeddings of the [CLS] tokens are concatenated as the input
    ...
    def __init__(self, model_name: str):
        ...
        self.head = nn.Linear(self.encoder.config.hidden_size * 3, 2)
        self.criterion = nn.CrossEntropyLoss()

    def forward(self,
                head_input_ids: torch.LongTensor,
                relation_input_ids: torch.LongTensor,
                tail_input_ids: torch.LongTensor,
                labels: Optional[torch.LongTensor] = None,
                return_dict: Optional[bool] = None):
        encoded_head = self.encoder(head_input_ids)[0][:, 0, :]  # Take <s> / [CLS] token embedding.
        encoded_relation = self.encoder(relation_input_ids)[0][:, 0, :]
        encoded_tail = self.encoder(tail_input_ids)[0][:, 0, :]
        embeds = torch.cat([encoded_head, encoded_relation, encoded_tail], dim=1)
...
def main():
    ...
    if training_args.do_train:
        ...
        for i in tqdm(range(len(df))):
            ...
            train_dataset[head_column].append(head)
            train_dataset[tail_column].append(tail)
            train_dataset[relation_column].append(rel)
    ...
    if training_args.do_eval:
        ...
        for i in tqdm(range(len(df))):
            if df.iloc[i]['split'] == 'dev':
                ...
                tail = df.iloc[i]['tail']
                rel = df.iloc[i]['relation']
                validation_dataset[head_column].append(head)
                validation_dataset[tail_column].append(tail)
                validation_dataset[relation_column].append(rel)
    ...
    if training_args.do_predict:
        ...
        for i in tqdm(range(len(df))):
            if df.iloc[i]['split'] == 'tst':
                ...
                tail = df.iloc[i]['tail']
                rel = df.iloc[i]['relation']
                test_dataset[head_column].append(head)
                test_dataset[tail_column].append(tail)
                test_dataset[relation_column].append(rel)
    ...
    def preprocess_function(examples):
        # Tokenize the texts
        result = {
            'head_input_ids': tokenizer(examples[head_column], padding=padding, max_length=max_seq_length,
                                        truncation=True).input_ids,
            'relation_input_ids': tokenizer(examples[relation_column], padding=padding, max_length=2,
                                            truncation=True).input_ids,
            'tail_input_ids': tokenizer(examples[tail_column], padding=padding, max_length=max_seq_length,
                                        truncation=True).input_ids,
