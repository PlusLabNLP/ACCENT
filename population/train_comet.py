...
class DataTrainingArguments:
    ...
    with_context: Optional[str] = field(
        default=True,
        metadata={
            "help": "Whether include dialogue context in model input or not"
        }
    )
    rel_first: Optional[str] = field(
        default=True,
        metadata={
            "help": "Whether have relation in thee beginning of tuple: (rel, ev1, ev2)"
        }
    )
...
def main():
    ...
    if training_args.do_train:
        ...
        for i in tqdm(range(len(df))):
            ...
            if rel == 'general Want':
                rel = 'gWant'
            if rel == 'general Effect':
                rel = 'gEffect'
            if rel == 'general React':
                rel = 'gReact'
            assert rel in additional_tokens_list, f'Invalid relation {rel}!'
            train_dataset[text_column].append(f'{head} {rel} [GEN]')
            train_dataset[output_column].append(tail)
